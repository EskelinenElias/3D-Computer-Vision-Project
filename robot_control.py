...
if __name__ == "__main__":
    ...
    # Use the last calibration view as the scene pose
    R_scene, _ = cv2.Rodrigues(calibration["rvecs"][-1])
    t_scene    = calibration["tvecs"][-1].ravel()

    # Detect objects in the first scene image
    ...
    # Plan pick-and-place for each block in order using the color-based API
    robot_state = {
    ...
    print(f"\n=== Planning on {os.path.basename(scene_paths[0])} ===")
    ...
    # Map each pick-and-place cycle to the cube colour it moves. Cycles occur
    # in BLOCK_ORDER, skipping any colour whose cube/target wasn't detected.
    moved_colors = [c for c in BLOCK_ORDER if c in cubes and c in targets]

    # Sample (pos, heading, held_color) densely along the plan so turns render
    # as arcs and the held cube visibly travels with the grabber.
    ...
    # ── Animated top-down world view ─────────────────────────────────────
    import matplotlib.animation as animation

    fig_top, ax_top = plt.subplots(figsize=(12, 6))

    # Static elements: target rings on the floor and cube starting positions
    ...
    # Fit axes to the full motion span so nothing clips during animation
    ...
    # ── Animated grabber path overlay on the scene image ─────────────────
