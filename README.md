...
2. **Camera calibration** — two methods, each in its own file:
   - **Zhang** ([camera_calibration_zhang.py](camera_calibration_zhang.py)) — two steps. `compute_intrinsics` recovers **K** from multiple checkerboard images at diverse board orientations (DLT homographies → closed-form K → joint LM refinement). `compute_extrinsics` recovers the scene pose **(R, t)** via OpenCV's `solvePnP` on a single scene checkerboard image.
   - **DLT** ([camera_calibration_dlt.py](camera_calibration_dlt.py)) — one step. Single image + ≥6 manually-picked 2D↔3D correspondences; projection-matrix decomposition yields K, R, t together.
...
4. **Robot control** — [robot_control.py](robot_control.py) — pick-and-place planner producing structured step dicts; separate translator serialises them to the `cmd; cmd; ...` string format the robot expects.
5. **Orchestration** — [main.py](main.py) — exposes the assignment-level `calibrate(intrinsic_imgs, extrinsic_img, method="zhang"|"dlt")` and `move_block(blocks, scene_img, calib)` functions.

Two notebooks:

- [demo.ipynb](demo.ipynb) — minimal end-to-end demo that calls `main.calibrate` + `main.move_block`.
- [walktrough.ipynb](walktrough.ipynb) — full walkthrough with per-step visualisations and an animated robot playback.
...
uv run python camera_calibration_dlt.py    # DLT pipeline (requires saved points)
...
- **`test-images/scene<N>/calibration/`** — one image per scene, checkerboard lying at a known world origin on the robot's table, camera at the scene's working pose. Used by `compute_extrinsics`. Pose diversity doesn't matter here (single-image PnP).
...
├── camera_calibration_zhang.py   # Zhang: compute_intrinsics + compute_extrinsics
...
├── walktrough.ipynb              # Full walkthrough with animations
├── test-images/
